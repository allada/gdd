...
var browser_protocol = JSON.parse(fs.readFileSync('./browser_protocol.json').toString());
// JS protocol overrides browser_protocol stuff.
var js_protocol = JSON.parse(fs.readFileSync('./js_protocol.json').toString());
...
'// Dispatchable Events'];
    if ('events' in domain) {
        for (var event of domain.events) {
            ...
            helperData.push(
`func (agent *${domainName}Agent) Fire${eventName}(event ${eventName}Event) {
    agent.conn.Send(shared.Notification{
        Method: "${domainName}.${event.name}",
        Params: event,
    })
}`);
            helperData.push(
`func (agent *${domainName}Agent) Fire${eventName}OnTarget(targetId string, event ${eventName}Event) {
...
function buildProperty(domainName, namespace, data, importsMap) {
    ...
    var isPointer = false;
    if (('optional' in data && data.optional)) {
